# Citi Bike - API Leaflet Visual Map
...
# What the code does
It downloads real-time station information from the Citi Bike API using d3.json. It loops through each station and creates a Leaflet marker showing its location, name, and capacity using Leaflet markers. It groups all markers into a single layer using layerGroup. It builds an interactive map centered on NYC with OpenStreetMap tiles using Leaflet map. It adds a layer control so users can toggle the bike station layer on and off.
